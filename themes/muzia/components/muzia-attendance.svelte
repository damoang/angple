    ...
    <div class="mx-auto max-w-2xl">
                ...
                {:else}
                    <div class="space-y-3">
                        <input type="text" bind:value={greeting} placeholder="출석 인사말을 입력하세요 (선택)"
                            class="w-full rounded-lg border bg-background px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-pink-400" />
                        <Button class="w-full bg-gradient-to-r from-pink-500 to-purple-600 py-3 text-white" onclick={checkIn} disabled={isChecking}>
        ...
        {:else}
            <div class="mb-6 overflow-hidden rounded-lg border bg-card p-6 text-center shadow-sm">
                ...
                <a href="/login"><Button class="bg-gradient-to-r from-pink-500 to-purple-600 text-white">로그인</Button></a>
            </div>
        {/if}
                    ...
                    {#each todayList as att, i}
                        <div class="flex items-center justify-between px-4 py-3">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-r from-pink-200 to-purple-200 text-xs font-medium text-purple-700">
                        ...
                        {#each Array(getDaysInMonth(calYear, calMonth)) as _, d}
                            ...
                            <button
                                class="relative flex h-12 flex-col items-center justify-center rounded-lg text-sm transition-all
                                    {selectedDate === dateStr ? 'bg-gradient-to-r from-pink-500 to-purple-600 text-white shadow-md' : ''}
