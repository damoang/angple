...
### [ ] 프론트엔드 인증 상태 관리 개선
- **현상**: 로그인 성공 후 UI에 로그인 상태가 반영되지 않음
- **상세**:
  - 로그인 API (`/api/v2/auth/login`) 요청 성공
  - 메인 페이지로 리다이렉트 성공
  - 하지만 사이드바에 "로그인이 필요합니다" 표시
  - `/api/v1/auth/refresh` → 500 Internal Server Error
  - `/api/v1/auth/profile` → 401 Unauthorized (토큰 미전달?)
- **원인 추정**:
  1. 로그인 성공 후 토큰이 스토어에 저장되지 않음
  2. 또는 페이지 리다이렉트 시 인증 상태 유실
  3. 또는 v1 auth 엔드포인트 문제
- **생성일**: 2026-02-04
...
### [x] Vite 프록시 설정 수정 (2026-02-04)
- `/api/v2/auth/login`만 v2로 처리 (나머지 auth는 v1)
- 로그인 API 요청 성공 확인 (curl, MCP Playwright)
