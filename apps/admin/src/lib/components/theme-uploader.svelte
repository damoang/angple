		...
		try {
			...
			xhr.addEventListener('load', () => {
				if (xhr.status >= 200 && xhr.status < 300) {
					...
					if (response.success) {
						uploadState = 'success';
						toast.success(`테마 "${response.manifest.name}"가 성공적으로 업로드되었습니다.`);
...
<Dialog bind:open={dialogOpen}>
	...
	<DialogContent class="sm:max-w-md">
		...
		<div class="space-y-4">
			...
			{#if !selectedFile}
				<div
					class="border-border hover:border-primary flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed p-8 transition-colors {isDragging ? 'border-primary bg-primary bg-opacity-5' : ''}"
				...
				<div class="bg-muted rounded-lg p-4">
					...
					{#if uploadState === 'uploading'}
						<div class="mt-4">
							...
							<p class="text-muted-foreground text-xs">업로드 중... {uploadProgress}%</p>
						</div>
					{/if}
