...
<div class="mx-auto max-w-2xl px-4 py-8">
	...
	<div class="space-y-6">
		...
		{#if data.profile}
						...
						use:enhance={() => {
							...
							return async ({ result }) => {
								if (result.type === 'success') {
									...
									nickError = (result.data?.error as string) || '변경에 실패했습니다.';
								}
							};
						}}
					>
						<div class="space-y-2">
							...
							<div class="flex gap-2">
								...
								/>
								<Button type="submit" variant="outline" disabled={data.nickChangeDaysLeft > 0}>
									변경
								</Button>
							</div>
						...
						use:enhance={() => {
							...
							return async ({ result }) => {
								if (result.type === 'success') {
									...
									emailError = (result.data?.error as string) || '변경에 실패했습니다.';
								}
							};
						}}
					>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>비밀번호를 변경합니다. 소셜 로그인으로만 가입한 경우 현재 비밀번호가 없을 수 있습니다.</CardDescription>
				</CardHeader>
				<CardContent>
					<form
						method="POST"
						...
						use:enhance={() => {
							...
							return async ({ result, update }) => {
								if (result.type === 'success') {
									...
									pwError = (result.data?.error as string) || '변경에 실패했습니다.';
								}
							};
						}}
						class="space-y-4"
						...
						use:enhance={() => {
							...
							return async ({ result }) => {
								if (result.type === 'success') {
									...
									profileError = (result.data?.error as string) || '저장에 실패했습니다.';
								}
							};
						}}
						class="space-y-4"
					>
						...
						<div class="flex items-center justify-between">
							<div>
								...
								<p class="text-muted-foreground text-xs">다른 회원에게 프로필을 공개합니다</p>
							</div>
						...
						<div class="flex items-center justify-between">
							<div>
								...
								<p class="text-muted-foreground text-xs">이메일 뉴스레터를 수신합니다</p>
							</div>
							<Switch id="mailling" name="mailling" checked={data.profile.mb_mailling === 1} />
						</div>
						{#if profileSuccess}
							<p class="flex items-center gap-1 text-xs text-green-600">
		...
		<div class="text-center">
			<a href="/member/leave" class="text-muted-foreground text-sm underline hover:text-red-600">
				회원 탈퇴
			</a>
		</div>
	</div>
