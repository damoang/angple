...
export const GET: RequestHandler = async ({ url, cookies }) => {
	...
	if (!targetId && !parentId) {
		return json({ status: 'error', message: 'targetId 또는 parentId가 필요합니다.' }, { status: 400 });
	}
	...
	try {
		...
		for (const row of reactions) {
			...
			result[row.target_id].push(
				parseReaction(row.reaction, row.reaction_count, isChosen)
			);
		}
...
export const POST: RequestHandler = async ({ request, cookies, getClientAddress }) => {
	...
	try {
		...
		if (!reaction || !targetId) {
			return json({ status: 'error', message: 'reaction과 targetId가 필요합니다.' }, { status: 400 });
		}
		...
		if (!VALID_REACTION_PATTERN.test(safeReaction)) {
			return json({ status: 'error', message: '유효하지 않은 리액션입니다.' }, { status: 400 });
		}
		...
		if (targetParts.length >= 3) {
			...
			try {
				...
				if (authorRows[0]?.mb_id === user.mb_id) {
					return json({ status: 'error', message: '자신의 글에 리액션을 할 수 없습니다.' }, { status: 403 });
				}
			} catch {
