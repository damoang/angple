<script lang="ts">
    ...
    } from '$lib/components/features/empathy/utils/index.js';
    ...
    let activeMode = $state<DiscoverMode>('hot');
    let topPeriod = $state<DiscoverTopPeriod>('24h');
    ...
    const topPeriods: { id: DiscoverTopPeriod; label: string }[] = [
        { id: '24h', label: '오늘' },
        { id: '7d', label: '이번 주' },
        { id: '30d', label: '이번 달' }
    ];

    const currentPosts = $derived.by((): DiscoverPost[] => {
        if (!data.discoverData) return [];
        const modeData = data.discoverData.modes[activeMode];
        if (!modeData) return [];
    ...
    const currentComments = $derived.by((): DiscoverComment[] => {
        if (!data.discoverData) return [];
        const modeData = data.discoverData.modes[activeMode];
        if (!modeData) return [];
    ...
    const hasComments = $derived.by((): boolean => {
        if (!data.discoverData) return false;
        const modeData = data.discoverData.modes[activeMode];
    ...
    function getPostId(post: DiscoverPost): number {
        return post.id;
    }
...
<div class="mx-auto max-w-4xl px-4 py-6">
    ...
    <div class="mb-4">
        <AdSlot position="discover-top" height="90px" slotKey="discover-top" />
    </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    {#if data.discoverData}
                        <span class="text-muted-foreground text-xs">
                            {data.discoverData.board_count}개 게시판
                        </span>
                    {/if}
                </div>
    ...
    <div class="mt-6">
        <AdSlot position="discover-bottom" height="90px" slotKey="discover-bottom" />
    </div>
</div>
