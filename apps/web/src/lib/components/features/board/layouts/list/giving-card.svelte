<script lang="ts">
	...
	const participantBadgeClass = $derived(() => {
		if (participantCount === 0) return 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400';
		if (participantCount < 20) return 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-300';
		if (participantCount < 50) return 'bg-orange-100 text-orange-700 dark:bg-orange-900 dark:text-orange-300';
	...
	const statusBadge = $derived(() => {
		switch (status) {
			case 'active':
				return { label: '진행중', class: 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' };
			case 'waiting':
				return { label: '대기중', class: 'bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300' };
			case 'paused':
				return { label: '일시정지', class: 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-300' };
			case 'ended':
				return { label: '종료', class: 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400' };
		}
	});
</script>
...
<!-- 나눔 카드 레이아웃 -->
<div
	class="bg-background border-border hover:border-primary/30 group cursor-pointer overflow-hidden rounded-lg border transition-all hover:shadow-md {status === 'ended' ? 'opacity-60' : ''}"
	{onclick}
	role="button"
	tabindex="0"
...
>
	...
	<div class="bg-muted relative aspect-video overflow-hidden">
		...
		<div class="absolute right-2 top-2">
			<span class="inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium {participantBadgeClass()}">
				<Users class="h-3 w-3" />
				{participantCount}
			</span>
		</div>
