    ...
    async function handleSubmit(): Promise<void> {
        ...
        try {
            ...
            if (err instanceof ApiRequestError && err.status === 409) {
                // bug/13818: #673 이후 잠긴 댓글에도 신고가 정상 기록된다(lock→409 차단 제거).
                // 이제 남는 409 는 "본인이 이미 신고한 댓글" 뿐이므로, 잠금/타인신고를 암시하던
                // 옛 문구 대신 "이미 신고하셨다"는 중립 안내로 표시한다(오류 아님·재시도 차단).
                alreadyHandled = true;
            } else {
    ...
    <DialogContent class="top-[5vh] max-h-[90dvh] translate-y-0 overflow-y-auto sm:max-w-md">
        ...
        {:else}
            ...
            <div class="space-y-3 py-3">
                ...
                <!-- 본인이 이미 신고한 댓글(409) 안내 (bug/13818, 오류 아님) -->
                {#if alreadyHandled}
                    <div class="bg-muted text-muted-foreground rounded-md p-3 text-sm">
                        이미 신고하신 댓글입니다. 접수된 신고는 정상적으로 검토 중이니 안심하셔도
                        됩니다.
                    </div>
                {:else if error}
                    <!-- 에러 메시지 -->
