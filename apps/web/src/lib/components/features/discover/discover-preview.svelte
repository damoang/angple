<script lang="ts">
    ...
    import type { DiscoverData, DiscoverMode, DiscoverPost } from '$lib/api/types.js';

    interface Props {
        prefetchData?: { data: DiscoverData } | unknown;
    }

    const { prefetchData }: Props = $props();

    const ssrData = prefetchData as { data: DiscoverData } | undefined;
    let discoverData = $state<DiscoverData | null>(ssrData?.data ?? null);
    ...
    const modePosts = $derived.by<DiscoverPost[]>(() => {
        if (!discoverData?.modes) return [];
        const modeData = discoverData.modes[activeMode];
        if (!modeData) return [];
    ...
    async function loadDiscoverData(): Promise<void> {
        if (requested || discoverData) {
            loading = false;
            return;
        }
        requested = true;
        ...
        try {
            const res = await fetch('/api/widgets/discover/data');
            if (res.ok) {
    ...
    function scheduleLoad(): void {
        if (!browser || requested || discoverData) return;
