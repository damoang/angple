<script lang="ts">
	import type { AIAnalysis, RecommendedDataWithAI } from '$lib/api/types.js';
...
>
	...
	<div class="min-w-0 flex-1">
		...
		<div class="mb-2 flex flex-wrap items-center justify-between gap-2">
			...
			<!-- 통계 (데스크톱에서만) -->
			{#if stats}
				<div class="hidden items-center gap-3 text-xs sm:flex" role="list" aria-label="트렌드 통계">
					<span class="flex items-center gap-1" role="listitem">
						<span>👍</span>
						<span class="font-medium text-foreground">{formatNumber(stats.total_recommends)}</span>
					</span>
					<span class="flex items-center gap-1" role="listitem">
						<span>💬</span>
						<span class="font-medium text-foreground">{formatNumber(stats.total_comments)}</span>
					</span>
