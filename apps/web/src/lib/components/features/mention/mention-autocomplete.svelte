		...
		{:else}
			{#each results as member, i (member.mb_id)}
				<button
					...
					class="hover:bg-accent flex w-full items-center gap-2 px-3 py-2 text-left text-sm transition-colors {i === selectedIndex ? 'bg-accent' : ''}"
					onmouseenter={() => (selectedIndex = i)}
					onmousedown={(e) => { e.preventDefault(); selectMember(member); }}
				>
					<div class="bg-primary text-primary-foreground flex size-7 shrink-0 items-center justify-center rounded-full text-xs">
						{member.mb_nick.charAt(0).toUpperCase()}
					</div>
					<div class="min-w-0 flex-1">
						<div class="flex items-center gap-1">
							<LevelBadge level={member.as_level} size="sm" />
							<span class="text-foreground truncate font-medium">{member.mb_nick}</span>
						</div>
