<script lang="ts">
    ...
    function handleToggle(widgetId: string) { widgetStore.toggleWidget(widgetId); }
    function handleDelete(widgetId: string) {
        if (confirm('이 위젯을 삭제하시겠습니까?')) { widgetStore.removeWidget(widgetId); }
    }
    function handleSelect(widgetId: string) { widgetStore.selectWidget(widgetId); }
    ...
    >
        {#each items as widget (widget.id)}
            ...
            >
                ...
                <button type="button" onclick={() => handleSelect(widget.id)} class="flex items-center gap-3 text-left">
                    <div class={cn('flex h-10 w-10 items-center justify-center rounded-lg', widget.enabled ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground')}>
                        <Icon class="h-5 w-5" />
                    </div>
                    <div>
                        <div class="font-medium">{getWidgetName(widget.type)}</div>
                        <div class="text-muted-foreground text-sm">{registry?.description ?? ''}</div>
                    </div>
                </button>

                <div>
                    {#if registry}
                        <Badge variant={getCategoryVariant(registry.category)}>{getCategoryLabel(registry.category)}</Badge>
                    {/if}
                </div>

                <div class="flex justify-center">
                    <Switch checked={widget.enabled} onCheckedChange={() => handleToggle(widget.id)} />
                </div>

                <div class="flex justify-end gap-1">
                    <Button variant="ghost" size="icon" onclick={() => handleSelect(widget.id)} title="설정">
                        <Settings class="h-4 w-4" />
                    </Button>
                    <Button variant="ghost" size="icon" onclick={() => handleDelete(widget.id)} title="삭제" class="hover:text-destructive">
