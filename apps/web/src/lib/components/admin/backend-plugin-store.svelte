<script lang="ts">
	...
	import { Settings, Power, PowerOff, Trash2, History, Download, RefreshCw } from '@lucide/svelte';
	...
	let settingsSchema = $state<Array<{ key: string; label: string; type: string; value: unknown; default_value: unknown }>>([]);
	...
	async function loadPlugins() {
		...
		try {
			...
			toast.error('백엔드 플러그인 목록을 불러오지 못했습니다. API 서버가 실행 중인지 확인하세요.');
		} finally {
			loading = false;
		}
	}
				...
				<div class="space-y-2">
					...
					{:else if setting.type === 'number'}
						<Input
							...
							oninput={(e: Event) => (settingValues[setting.key] = Number((e.target as HTMLInputElement).value))}
						/>
					{:else}
						<Input
							id={setting.key}
							type="text"
							value={settingValues[setting.key] as string}
							oninput={(e: Event) => (settingValues[setting.key] = (e.target as HTMLInputElement).value)}
						/>
					{/if}
				</div>
			{/each}
					...
					{#each events as event (event.id)}
						<div class="flex items-center gap-3 rounded-lg border p-3">
							...
							<div class="flex-1">
								...
								{#if event.actor_id}
									<span class="text-muted-foreground text-xs"> · {event.actor_id}</span>
								{/if}
							</div>
							{#if event.details}
