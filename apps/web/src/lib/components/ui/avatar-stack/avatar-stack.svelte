		...
		{#each visibleItems as item (item.mb_id)}
			...
			{#if iconUrl}
				<img
					...
					alt={nick}
					title={nick}
					class="{sizeClass} rounded-full object-cover ring-2 ring-background"
					onerror={(e) => {
				...
				/>
				<div
					class="bg-muted text-muted-foreground hidden {sizeClass} items-center justify-center rounded-full ring-2 ring-background {textSize} font-medium"
					title={nick}
				>
			...
			{:else}
				<div
					class="bg-muted text-muted-foreground flex {sizeClass} items-center justify-center rounded-full ring-2 ring-background {textSize} font-medium"
					title={nick}
				>
					{nick.charAt(0).toUpperCase()}
				</div>
