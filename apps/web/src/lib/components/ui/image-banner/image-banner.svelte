<script lang="ts">
    ...
    async function fetchBanner() {
        ...
        try {
            const response = await fetch(`${API_BASE}/api/v1/serve/banners?position=${encodeURIComponent(position)}`);
...
>
    {#if loading}
        <!-- 로딩 플레이스홀더 -->
        <div class="flex animate-pulse items-center justify-center rounded-lg bg-slate-100 dark:bg-slate-800" style="min-height: {height};">
