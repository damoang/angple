...
interface Settings {
	...
	activatedAt?: string;
	themes: Record<string, { settings: Record<string, any> }>;
	version: string;
}
...
export class JsonSettingsProvider implements SettingsProvider {
	...
	async getThemeSettings(themeId: string): Promise<Record<string, any>> {
		const settings = await this.read();
		return settings.themes[themeId]?.settings || {};
	}

	async setThemeSettings(themeId: string, themeSettings: Record<string, any>): Promise<void> {
		await this.acquireLock();
		try {
			const settings = await this.read();
