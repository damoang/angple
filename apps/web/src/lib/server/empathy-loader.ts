...
import type { EmpathyDataWithAI, EmpathyPeriod } from '$lib/api/types';
import { env } from '$env/dynamic/private';

const RECOMMENDED_CACHE_DIR =
    env.RECOMMENDED_CACHE_DIR || '/home/damoang/www/data/cache/empathy';
...
/** 인메모리 캐시 (period별) */
const cache = new Map<string, { data: EmpathyDataWithAI; timestamp: number }>();
 ...
 * KST(한국 표준시) 기준으로 계산
 */
export function getDefaultPeriod(): EmpathyPeriod {
 ...
 * 추천글 캐시 파일을 읽어 반환
 */
export async function loadEmpathyData(
    period: EmpathyPeriod
): Promise<EmpathyDataWithAI | null> {
    // 인메모리 캐시 확인
    ...
    try {
        ...
        const data: EmpathyDataWithAI = JSON.parse(content);

        cache.set(period, { data, timestamp: Date.now() });
        return data;
    } catch (err) {
        console.error('[empathy-loader] 캐시 파일 읽기 실패:', err);
        // stale 캐시라도 반환
