...
class HookRegistry {
	 ...
	 * // 결과: "🔥 Hello"
	 */
	async applyFilter<T = any>(name: string, value: T, ...args: any[]): Promise<T> {
		const hookList = this.hooks.get(name);
		...
		for (const hook of hookList) {
			try {
				...
				currentValue = result !== undefined ? result : currentValue;
			} catch (error) {
				console.error(
 ...
 */
export async function applyFilter<T = any>(name: string, value: T, ...args: any[]): Promise<T> {
