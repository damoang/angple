...
export const instagram: EmbedPlatform = {
    ...
    render(info: EmbedInfo): string {
        ...
        const width = info.maxWidth || 540;
        const height = info.platform === 'instagram-reel' ? 900 : 650;

        return `<iframe src="${embedUrl}" title="Instagram embed" width="${width}" height="${height}" frameborder="0" scrolling="no" allowtransparency="true" allowfullscreen></iframe>`;
    }
};
